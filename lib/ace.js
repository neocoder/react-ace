  ...
  _createClass(ReactAce, [{
    ...
    value: function componentDidMount() {
      ...
      var _props = this.props,
          ...
          onAnnotationsChange = _props.onAnnotationsChange;
      ...
      var edSession = this.editor.getSession();
      edSession.setAnnotations(annotations || []);
      edSession.on('changeAnnotation', function () {
        if (onAnnotationsChange) {
          onAnnotationsChange(edSession.getAnnotations().map(function (ann) {
            ann.row++;ann.column++;ann.id = ann.row + ':' + ann.column;return ann;
          }));
        }
      });
